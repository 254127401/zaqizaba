...
class Coin():
    ...
    def content(url,title,author,times,sort,img_url,brief,categories):
        print(url,title,author,times,sort,img_url,brief,categories)
        print('文章：%s'%url)
        req=requests.get(url,verify=False).text
        content='<div class="pre_text">'+Coin.qwbzj(req,'<div class="pre_text">','<div class="snsbox">').strip()
    ...
    def news():
        ...
        url_xpath='//*[@class="list"]/article/a'
        briefs_xpath='//*[@class="flexbox"]/div[2]/h3/text()'
        img_xpath='//*[@class="flexbox"]/div/img'
        page=1
        while True:
            url_get='https://venturetimes.jp/cryptocurrency/coin/page/%d'%page
